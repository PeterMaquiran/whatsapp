...
**Product identity:** credential accounts (handle/email + password and/or Google; Keycloak later on the same IdP seam) that can be logged in on **multiple devices at once**, like Facebook Messenger. It is **not** WhatsApp (phone number + primary device + QR-linked companions). We still copy WhatsApp-like send/receipt/offline UX.
...
| [12 Observability](./12-observability.md) | OTel Collector, SLOs, Alertmanager, Tempo, Loki, Prometheus, Pyroscope, Grafana as code |
| [13 Learning roadmap](./13-learning-roadmap.md) | Fundamentals → distributed → scale → AI, mapped to this repo |
| [14 Decisions](./14-decisions.md) | ADRs: Socket.IO first, NestJS gateway host, Postgres first, credential multi-device, IdP (Google / later Keycloak) not session, no E2E in v1, TUS media, OTel + SLOs + Alertmanager + Pyroscope, OSS local consoles |
| [15 Sprints](./15-sprints.md) | GitHub milestones and issues: Next.js + NestJS gateway monorepo, Sprint 1 → 10, backlog |
| [16 Local consoles](./16-local-consoles.md) | Redis Insight, Adminer, Grafana; no custom admin UIs |
